...
<div class="min-h-screen bg-gradient-to-br from-brand-950 via-brand-900 to-emerald-900 flex flex-col" x-data="consultationForm()">
    ...
    <div class="flex items-center justify-between px-6 py-4 border-b border-white/10">
        <a href="{{ route('home') }}" class="flex items-center gap-2.5">
            ...
            <span class="font-heading font-bold text-white">Temu<span class="text-teal-300">Dokter</span></span>
        </a>
        <div class="text-white/60 text-sm">Formulir Konsultasi</div>
    </div>
                ...
                <form action="{{ route('patient.store') }}" method="POST" enctype="multipart/form-data" id="consultation-form">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="email" id="email" name="email"
                                           ...
                                           value="{{ old('email') }}"
                                           x-model="form.email">
                                    <p class="text-xs text-slate-500 mt-1">Digunakan untuk mengirim hasil riwayat konsultasi, resep, dan surat sakit.</p>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div>
                                    ...
                                    <select name="gender" id="gender" required
                                            class="form-select @error('gender') border-rose-400 @enderror" x-model="form.gender">
                            ...
                            <div class="col-span-1 sm:col-span-2 space-y-4 pt-2">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <select id="province" class="form-select" x-model="selectedProvince" @change="onProvinceChange()" required>
                                            <option value="">Pilih Provinsi...</option>
                                            <template x-for="p in provinces" :key="p.id">
                                                <option :value="p.id + '|' + p.name" x-text="p.name"></option>
                                            </template>
                                        </select>
                                    ...
                                    <div>
                                        ...
                                        <select id="city" class="form-select" x-model="selectedCity" @change="onCityChange()" :disabled="!selectedProvince" required>
                                            <option value="">Pilih Kab/Kota...</option>
                                            <template x-for="c in cities" :key="c.id">
                                                <option :value="c.id + '|' + c.name" x-text="c.name"></option>
                                            </template>
                                        </select>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <select id="district" class="form-select" x-model="selectedDistrict" @change="onDistrictChange()" :disabled="!selectedCity" required>
                                            <option value="">Pilih Kecamatan...</option>
                                            <template x-for="d in districts" :key="d.id">
                                                <option :value="d.id + '|' + d.name" x-text="d.name"></option>
                                            </template>
                                        </select>
                                    ...
                                    <div>
                                        ...
                                        <select id="village" class="form-select" x-model="selectedVillage" @change="onVillageChange()" :disabled="!selectedDistrict" required>
                                            <option value="">Pilih Kel/Desa...</option>
                                            <template x-for="v in villages" :key="v.id">
                                                <option :value="v.id + '|' + v.name" x-text="v.name"></option>
                                            </template>
                                        </select>
                    ...
                    <div x-show="step === 3" x-transition:enter="transition ease-out duration-300"
                        ...
                        <div class="px-8 pt-8 pb-6 border-b border-slate-100">
                            ...
                            <p class="text-slate-500 text-sm">Periksa kembali data Anda sebelum melanjutkan ke pembayaran.</p>
                        </div>
                        <div class="px-8 py-6">
                            <div class="bg-slate-50 rounded-2xl p-5 space-y-4 mb-6">
                                <div class="grid grid-cols-2 gap-4 text-sm">
                                    ...
                                    <div class="col-span-2">
                                        <p class="text-slate-500 text-xs mb-1">Alamat Domisili Lengkap</p>
                                        <p class="font-semibold text-slate-800" x-text="[
                                            form.rt_rw,
                            ...
                            {{-- Fee info --}}
                            <div class="flex items-center justify-between p-4 bg-brand-50 border border-brand-100 rounded-2xl mb-6">
                                <div>
                                    <p class="text-sm font-semibold text-slate-800">Biaya Konsultasi</p>
                                    <p class="text-xs text-slate-500">Sesi 15 menit dengan dokter</p>
                                </div>
                                <div class="text-2xl font-bold text-brand-700 font-heading">Rp 25.000</div>
                            </div>
                        ...
                        <div class="px-8 pb-8 flex items-center justify-between">
                            ...
                            <button type="submit" class="btn-primary btn-lg" :disabled="!agreedToConsent || submitting" :class="(!agreedToConsent || submitting) ? 'opacity-50 cursor-not-allowed' : ''">
                                ...
                                <span x-text="submitting ? 'Memproses...' : 'Submit & Lanjut ke Pembayaran'"></span>
                            </button>
                        </div>
                    </div>
                    ...
                    <div x-show="showConsentModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6" style="display: none;">
                        ...
                        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col relative z-10"
                            ...
                            <div class="p-5 sm:px-8 overflow-y-auto scrollbar-thin flex-1 text-sm text-slate-700 space-y-4">
                                <p class="font-bold text-center text-slate-800 uppercase mb-4">(INFORMED CONSENT LAYANAN TEMU DOKTER)</p>
                                
                                <p class="font-bold text-slate-800 uppercase">Tujuan</p>
                                <p>Layanan Temu Dokter merupakan pelayanan konsultasi kesehatan jarak jauh maupun pemeriksaan medis tatap muka yang dilakukan oleh dokter melalui media teknologi informasi dan komunikasi. Layanan ini bertujuan untuk memberikan edukasi kesehatan, penilaian awal terhadap keluhan pasien, pertimbangan diagnosis kerja, rekomendasi pemeriksaan lanjutan apabila diperlukan, serta saran tata laksana sesuai dengan informasi yang disampaikan oleh pasien.</p>
                                
                                <p class="font-bold text-slate-800 uppercase mt-4">Keikutsertaan</p>
                                <p>Keikutsertaan Anda dalam layanan Temu Dokter bersifat sukarela dan atas kehendak pribadi tanpa paksaan dari pihak mana pun.</p>
                                <p>Anda berhak untuk tidak melanjutkan atau menghentikan konsultasi apabila tidak menyetujui syarat dan ketentuan yang berlaku. Dengan melanjutkan konsultasi, Anda dianggap telah membaca, memahami, dan menyetujui seluruh ketentuan yang berlaku dalam layanan ini.</p>
                                
                                <p class="font-bold text-slate-800 uppercase mt-4">Durasi dan Prosedur</p>
                                <p>Sesi konsultasi berlangsung sesuai ketentuan yang berlaku pada aplikasi.</p>
                                <p>Selama sesi berlangsung, Anda dapat menyampaikan keluhan, riwayat penyakit, riwayat alergi, riwayat pengobatan, hasil pemeriksaan penunjang, maupun informasi kesehatan lain yang relevan melalui fitur yang tersedia pada aplikasi.</p>
                                <p>Dokter akan memberikan penilaian medis berdasarkan informasi yang Anda sampaikan. Apabila diperlukan, dokter dapat memberikan resep obat, anjuran pemeriksaan penunjang, rujukan, atau rekomendasi untuk melakukan pemeriksaan langsung di fasilitas kesehatan.</p>
                                <p>Pasien memahami bahwa kualitas rekomendasi medis sangat bergantung pada kelengkapan, keakuratan, dan kejujuran informasi yang diberikan selama konsultasi.</p>

                                <p class="font-bold text-slate-800 uppercase mt-4">Manfaat</p>
                                <p>Layanan Temu Dokter dapat memberikan manfaat berupa:</p>
                                <ul class="list-disc pl-5 space-y-1">
                                    <li>Edukasi dan informasi kesehatan.</li>
                                    <li>Penilaian awal terhadap kondisi kesehatan pasien.</li>
                                    <li>Pertimbangan diagnosis kerja berdasarkan informasi yang tersedia.</li>
                                    <li>Saran tata laksana dan pengobatan sesuai indikasi medis.</li>
                                    <li>Rekomendasi pemeriksaan lanjutan atau rujukan apabila diperlukan.</li>
                                </ul>

                                <p class="font-bold text-slate-800 uppercase mt-4">Risiko dan Ketidaknyamanan</p>
                                <p>Pasien memahami dan menyetujui bahwa:</p>
                                <ul class="list-disc pl-5 space-y-1">
                                    <li>Diagnosis, saran, maupun terapi yang diberikan dokter didasarkan pada data dan informasi yang disampaikan oleh pasien.</li>
                                    <li>Risiko alergi, efek samping obat, interaksi obat, atau respons tubuh yang tidak diharapkan dapat terjadi meskipun telah diberikan sesuai standar profesi dan berdasarkan informasi yang tersedia saat konsultasi.</li>
                                    <li>Pasien wajib menyampaikan secara lengkap dan jujur mengenai riwayat alergi, penyakit yang sedang atau pernah diderita, kehamilan, obat-obatan yang sedang dikonsumsi, serta informasi medis lain yang relevan.</li>
                                    ...
                                    <li>Gangguan jaringan internet, keterlambatan respons, gangguan sistem aplikasi, atau kendala teknis lainnya dapat memengaruhi kelancaran layanan dan berada di luar kendali dokter.</li>
                                </ul>

                                <p class="font-bold text-slate-800 uppercase mt-4">Kerahasiaan Data</p>
                                <ul class="list-disc pl-5 space-y-1">
                                    <li>Data pribadi dan informasi kesehatan pasien akan dijaga kerahasiaannya sesuai dengan ketentuan peraturan perundang-undangan yang berlaku.</li>
                                    <li>Data dapat disimpan dan dikelola oleh penyelenggara aplikasi untuk keperluan pelayanan kesehatan, dokumentasi medis, peningkatan mutu layanan, pemenuhan kewajiban hukum, serta kepentingan lain yang diperbolehkan oleh peraturan perundang-undangan.</li>
                                    <li>Pasien memahami bahwa data dapat diakses oleh tenaga kesehatan, fasilitas pelayanan kesehatan, atau pihak lain yang berwenang sesuai ketentuan hukum yang berlaku.</li>
                                </ul>

                                <p class="font-bold text-slate-800 uppercase mt-4">Pembiayaan</p>
                                <p>Biaya yang timbul dari penggunaan layanan Temu Dokter menjadi tanggung jawab pengguna sesuai tarif yang berlaku.</p>

                                <p class="font-bold text-slate-800 uppercase mt-4">Klarifikasi</p>
                                <p>Apabila Anda memiliki pertanyaan, memerlukan penjelasan lebih lanjut, atau membutuhkan klarifikasi terkait layanan Temu Dokter, Anda dapat menghubungi email : admintemudokter@gmail.com atau kontak resmi yang tersedia pada website.</p>

                                <p class="font-bold text-slate-800 uppercase mt-4">Pernyataan Persetujuan</p>
                                <p>Dengan melanjutkan penggunaan layanan Temu Dokter, saya menyatakan bahwa:</p>
                                <ul class="list-disc pl-5 space-y-1 mb-4">
                                    ...
                                    <li>Saya memahami bahwa layanan ini memiliki keterbatasan karena dilakukan tanpa pemeriksaan fisik secara langsung.</li>
                                    <li>Saya memahami bahwa diagnosis, saran, resep, dan rekomendasi yang diberikan dokter didasarkan pada informasi yang saya sampaikan selama konsultasi.</li>
                                    <li>Saya memahami bahwa dokter dapat menyarankan pemeriksaan langsung atau rujukan ke fasilitas kesehatan apabila kondisi saya memerlukan evaluasi lebih lanjut.</li>
                                    <li>Saya membebaskan dokter dan penyelenggara layanan dari tuntutan yang timbul akibat informasi yang tidak lengkap, tidak akurat, atau tidak benar yang saya berikan selama konsultasi, sepanjang dokter telah menjalankan pelayanan sesuai standar profesi dan ketentuan yang berlaku.</li>
                                </ul>
...
function consultationForm() {
    return {
        ...
        onProvinceChange() {
            ...
            this.selectedCity = ''; this.selectedDistrict = ''; this.selectedVillage = '';
        ...
        onCityChange() {
            ...
            const [id, name] = this.selectedCity.split('|');
            this.form.city = name;
        ...
        async fetchVillages(distId) {
            ...
            } catch(e) { console.error('Gagal memuat kelurahan', e); }
        },
        onVillageChange() {
            ...
            this.form.village = name;
        },
        nextStep() {
            if (this.step === 1) {
                if (!this.form.full_name || !this.form.whatsapp_number || !this.form.age || !this.form.gender || (!this.form.occupation && !this.form.no_occupation) || !this.form.province || !this.form.city || !this.form.district || !this.form.village || !this.form.rt_rw) {
                    alert('Mohon lengkapi semua field Data Diri dan Alamat Lengkap.');
                    return;
                }
            }
            if (this.step === 2) {

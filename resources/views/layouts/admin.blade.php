...
<head>
    ...
    <title>@yield('title', 'Dashboard') – Temu Dokter Admin</title>
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="absolute z-50 md:relative h-full w-60 bg-brand-950 flex-shrink-0 flex flex-col"
            ...
            <div class="flex items-center gap-2.5 px-6 py-5 border-b border-white/10">
                <img src="{{ asset('images/logo.png') }}?v={{ time() }}" alt="Temu Dokter Logo" class="h-10 w-auto rounded-lg shadow-sm">
                <div>
            ...
            <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                ...
                <a href="{{ route('admin.treatment.index') }}"
                   class="{{ request()->routeIs('admin.treatment.*') ? 'bg-brand-600 text-white' : 'text-white/70 hover:bg-white/10 hover:text-white' }} flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7"/></svg>
                ...
                <a href="{{ route('admin.revenue.history.index') }}"
                   class="{{ request()->routeIs('admin.revenue.history.*') ? 'bg-brand-600 text-white' : 'text-white/70 hover:bg-white/10 hover:text-white' }} flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    Riwayat Pendapatan
                </a>
            </nav>
            ...
            <header class="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between flex-shrink-0">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500">@yield('page_subtitle', 'Temu Dokter Admin Panel')</p>
                    </div>
                </div>
